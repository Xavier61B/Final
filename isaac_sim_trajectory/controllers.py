...
class Controller:
    ...
    def v_map(self, matrix):
        w_hat = matrix[0:3,0:3]
        p = matrix[0:3,3].reshape(3,1)

        w = np.array([-w_hat[1,2], w_hat[0,2], -w_hat[0,1]]).reshape(3,1)

        return np.vstack((w,p))

    def adjoint(self, g):
        R = g[0:3,0:3]
        p = g[0:3,3]
        p_hat = np.array([[0, -p[2], p[1]],[p[2], 0, -p[0]],[-p[1], p[0], 0]])
        top = np.hstack((R, p_hat @ R))
        bot = np.hstack((np.zeros((3,3)),R))
...
class WorkspaceVelocityController(Controller):
    ...
    def step_control(self, target_position, target_velocity, target_acceleration):
        ...
        gst = tf.transformations.quaternion_matrix(pose["orientation"])
        tp = target_position
        tp[2] = target_position[2] + 0.14
        print(pose["position"].z - tp[2])
        gst[0:3,3] = np.array(pose["position"]).reshape(3)
        gsd = tf.transformations.quaternion_matrix(tp[3:])
        gsd[0:3,3] = np.array(tp[0:3]).reshape(3)

        xi_td = self.v_map(logm(np.linalg.inv(gst) @ gsd))
        Adj_gst = self.adjoint(gst)
        U = self.Kp @ (Adj_gst @ xi_td) + target_velocity.reshape(6,1)

        control_input = self._kin.jacobian_pseudo_inverse() @ U
...
class PDJointVelocityController(Controller):
    ...
    def step_control(self, target_position, target_velocity, target_acceleration):
        ...
        """
        ep = target_position - [self._limb.joint_angles()["right_j" + str(i)] for i in range(7)]
        ev = target_velocity - [self._limb.joint_velocities()["right_j" + str(i)] for i in range(7)]
        control_input = self.Kp @ ep + self.Kv @ ev
        self._limb.set_joint_velocities(joint_array_to_dict(control_input, self._limb))
...
class PDJointTorqueController(Controller):
    ...
    def step_control(self, target_position, target_velocity, target_acceleration):
        ...
        M = self._kin.inertia(self._limb.joint_angles())
        C = self._kin.coriolis(self._limb.joint_angles(), self._limb.joint_velocities())
        N = 0.01 * self._kin.gravity(self._limb.joint_angles())
        vel = np.array([self._limb.joint_velocities()["right_j" + str(i)] for i in range(7)])
        ep = -target_position + [self._limb.joint_angles()["right_j" + str(i)] for i in range(7)]
        ev = -target_velocity + [self._limb.joint_velocities()["right_j" + str(i)] for i in range(7)]
        A = (target_acceleration - self.Kv @ ev - self.Kp @ ep )
        D = (M @ A).reshape(7,1)
        control_input = D + C + N
...
class RLController(Controller):
    ...
    def step_control(self, target_position, target_velocity, target_acceleration):
        ...
        j_pos = torch.tensor([self._limb.joint_angles()["right_j" + str(i)] for i in range(7)])
        j_vel = torch.tensor([self._limb.joint_velocities()["right_j" + str(i)] for i in range(7)])
        for i in range(7):
            ind = 2 * i
        ...
        self._limb.set_joint_torques(joint_array_to_dict(action, self._limb))
